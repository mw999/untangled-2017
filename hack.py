...
class Hack():
        ...
        if(keys[pygame.K_LSHIFT] and self.count%4 == 0):
            ...
            if(keys[pygame.K_SPACE]):
                for i in range(0,30):
                    ...
                    spell = player_module.Spell(player,velocity,client_module.projectile_images[player.current_spell])
    ...
    def Nuke(self):
        spell = player_module.Spell(self.player,(0,0),client_module.projectile_images[self.player.current_spell],(-1337,-1337))
        if self.canKeys >= 1:
            return
        spell.render()
        if self.canKeys > 1:
